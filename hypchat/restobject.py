...
class Linker(object):
    ...
    def __call__(self, expand=None, **kwargs):
        ...
        rv = self._obj_from_text(
            self._requests.get(self.url, params=params).text, self._requests)
...
class RestObject(dict):
    ...
    def __getattr__(self, name):
        if name in self.get('links', {}):
            return Linker(self['links'][name], parent=self,
...
class Room(RestObject):
    ...
    def save(self):
        data = {}
        for key in (
        'name', 'privacy', 'is_archived', 'is_guest_accessible', 'topic'):
            data[key] = self[key]
        data['owner'] = {
        'id': self['owner']['id']
        }
...
class Collection(object):
    ...
    def contents(self, **kwargs):
        ...
        while hasattr(page, 'next'):
            ...
            # There's a bug where we run out of items, but 'next' is still
            # supplied. So we only call next() if the current page returns
            # items:
            if not page['items']:
                return
            page = page.next(**ops)
...
_urls_to_objects[
    re.compile(r'https://[^/]+/v2/room/[^/]+/member$')] = MemberCollection


class UserCollection(RestObject, Collection):
    def create(self, name, email, title=None, mention_name=None,
               is_group_admin=False, timezone='UTC', password=None):
        """
        Creates a new user.
        """
        data = {
...
class RoomCollection(RestObject, Collection):
    def create(self, name, owner=Ellipsis, privacy='public',
               guest_access=True):
        """
        Creates a new room.
        """
...
_urls_to_objects[
    re.compile(r'https://[^/]+/v2/room/[^/]+/webhook$')] = WebhookCollection
...
_urls_to_objects[
    re.compile(r'https://[^/]+/v2/emoticon$')] = EmoticonCollection
...
_urls_to_objects[
    re.compile(r'https://[^/]+/v2/room/[^/]+/webhook/[^/]+$')] = Webhook
...
_urls_to_objects[
    re.compile(r'https://[^/]+/v2/room/[^/]+/history$')] = HistoryCollection
